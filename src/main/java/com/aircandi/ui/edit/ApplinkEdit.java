...
public class ApplinkEdit extends BaseEntityEdit {
	...
	public void initialize(Bundle savedInstanceState) {
		...
		mSpinnerItemResId = Aircandi.themeTone.equals(ThemeTone.DARK) ? R.layout.spinner_item_dark : R.layout.spinner_item_light;
	...
	public void draw(View view) {
		...
		if (mEditing) {
			...
			if (mTypePicker.getAdapter() == null) {
				...
				mApplinkSearchStrings.add(StringManager.getString(R.string.hint_applink_type));
				initializeTypeSpinner(mApplinkSearchStrings);
			}
		}
		if (applink.type != null) {

			UI.setVisibility(findViewById(R.id.app_id), View.GONE);
			UI.setVisibility(findViewById(R.id.app_url), View.GONE);

			if (applink.type.equals(Constants.TYPE_APP_WEBSITE)) {
				UI.setVisibility(findViewById(R.id.app_url), View.VISIBLE);
				mMissingResId = R.string.error_missing_applink_url_website;
			}
			else if (applink.type.equals(Constants.TYPE_APP_EMAIL)) {
				UI.setVisibility(findViewById(R.id.app_id), View.VISIBLE);
	...
	public void onTestButtonClick() {
		if (validate()) {
			gather();
			Aircandi.dispatch.shortcut(this, mEntity.getShortcut(), null, null, null);
		}
	}
	...
	private void verifyAndExit() {
		...
		new AsyncTask() {
			...
			protected void onPreExecute() {
				mBusy.showBusy(BusyAction.ActionWithMessage, R.string.progress_applink_verify);
			}

			@Override
			protected Object doInBackground(Object... params) {
				Thread.currentThread().setName("AsyncApplinkVerify");
				ModelResult result = Aircandi.getInstance().getEntityManager().verifyApplink(mEntity, ServiceConstants.TIMEOUT_APPLINK_SEARCH, true);
				return result;
			}
			...
			protected void onPostExecute(Object response) {
				...
				if (result.serviceResponse.responseCode == ResponseCode.SUCCESS) {
					...
					/*
					 * Applink verification can return additional links.
					 */
					...
					if (applinks.size() > 0) {
						...
						/*
						 * Round trip to service strips our local properties. We need to make sure the photo local
						 * properties are restored.
						 */
						validated.photo = mEntity.photo;
					}

					/* Validation failed */
					if (applinks.size() == 0 || (validatedDate != null && validatedDate == -1)) {
						Applink applink = (Applink) mEntity;
						String message = StringManager.getString(R.string.error_applink_verification_failed) + " "
						...
						finish();
						Aircandi.getInstance().getAnimationManager().doOverridePendingTransition(ApplinkEdit.this, TransitionType.FORM_TO_PAGE);
					}
				}
			}
		}.execute();
	...
	protected boolean validate() {
		...
		if (mEntity.type == null) {
			Dialogs.alertDialog(android.R.drawable.ic_dialog_alert
					, null
					, StringManager.getString(R.string.error_missing_applink_type)
					, null
					, this
					, android.R.string.ok
					, null, null, null, null);
		...
		if (!mEditing && mAppId != null && mAppId.getText().length() == 0 && findViewById(R.id.app_id).getVisibility() == View.VISIBLE) {
			Dialogs.alertDialog(android.R.drawable.ic_dialog_alert
					, null
