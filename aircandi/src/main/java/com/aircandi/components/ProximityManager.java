...
public class ProximityManager {
	...
	public void scanForWifi(final ScanReason reason) {
		 ...
		 */
		if (Aircandi.applicationContext == null) return;
			...
			if (!Aircandi.usingEmulator) {
				...
				Aircandi.applicationContext.registerReceiver(new BroadcastReceiver() {
					...
					public void onReceive(Context context, Intent intent) {
						...
						final String testingBeacons = Aircandi.settings.getString(StringManager.getString(R.string.pref_testing_beacons), "natural");
						...
						final String testingBeacon = Aircandi.settings.getString(StringManager.getString(R.string.pref_demo_beacons), "{\"name\":\"natural\"}");
	...
	public synchronized ServiceResponse getEntitiesByProximity() {
		...
		String installId = Aircandi.getinstallId();
		...
		serviceResponse = mEntityCache.loadEntitiesByProximity(beaconIds
				, Aircandi.getInstance().getEntityManager().getLinks().build(LinkProfile.LINKS_FOR_BEACONS)
				, cursor
				, installId
				, Aircandi.stopwatch1);
	...
	public synchronized ServiceResponse getEntitiesNearLocation(AirLocation location) {
		...
		for (Entity entity : Aircandi.getInstance().getEntityManager().getPlaces(false, true)) {
			Place place = (Place) entity;
			excludePlaceIds.add(place.id);
		}

		ServiceResponse serviceResponse = mEntityCache.loadEntitiesNearLocation(location
				, Aircandi.getInstance().getEntityManager().getLinks().build(LinkProfile.LINKS_FOR_PLACE)
				, excludePlaceIds);
